...
#include "s5k5e8yxb6mipiraw_Sensor.h"

/****************************Modify following Strings for debug****************************/
#define PFX "s5k5e8yxb6_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K5E8YX_B6_SENSOR_ID,
	...
	.i2c_addr_table = {0x20, 0x30, 0x5a, 0xff},
};
...
static kal_uint32 open(void)
{
	...
	if (first_flag == 1) {
		LOG_INF("modify device I2C address to 0x30\n");
		write_cmos_sensor(0x0107, 0x30);
		first_flag = 0;
	}
	
	//sensor have two i2c address 0x20 0x5a & 0x30 0x20, we should detect the module used i2c address
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
...
UINT32 S5K5E8YX_B6_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K5E8YX_B6_MIPI_RAW_SensorInit	*/
